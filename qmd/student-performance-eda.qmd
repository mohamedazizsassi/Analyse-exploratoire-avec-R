...
# Analyse multivariée exploratoire (ACP) {#sec-acp}
...
# Retire les colonnes entièrement NA et supprime les lignes incomplètes
pca_numeric <- pca_df %>%
	select(-subject, -school, -sex) %>%
	select(where(~ !all(is.na(.x))))

pca_complete <- bind_cols(
	pca_df %>% select(subject, school, sex),
	pca_numeric
) %>%
	tidyr::drop_na()

pca_model <- stats::prcomp(
	pca_complete %>% select(-subject, -school, -sex),
	center = TRUE,
	scale. = TRUE
)

pca_var <- (pca_model$sdev^2) / sum(pca_model$sdev^2)
pca_var_tbl <- tibble(
	Composante = paste0("PC", seq_along(pca_var)),
	`Variance expliquée` = pca_var,
	`Variance cumulée` = cumsum(pca_var)
)

pca_scores <- as_tibble(pca_model$x[, 1:2, drop = FALSE]) %>%
	bind_cols(pca_complete %>% select(subject, school, sex))

pca_loadings <- as_tibble(pca_model$rotation[, 1:2, drop = FALSE], rownames = "variable")
```
...
pca_var_tbl %>%
	mutate(
		`Variance expliquée` = scales::percent(`Variance expliquée`, accuracy = 0.1),
		`Variance cumulée` = scales::percent(`Variance cumulée`, accuracy = 0.1)
	) %>%
	slice_head(n = 6) %>%
	kable(caption = "Variance expliquée par les premières composantes")
```

## Projection des individus (PC1–PC2)

```{r, label="fig-pca-subject", fig.cap="ACP : projection des individus (PC1–PC2) colorée par discipline."}
ggplot(pca_scores, aes(x = PC1, y = PC2, color = subject)) +
	geom_point(alpha = 0.35) +
	labs(x = "PC1", y = "PC2", color = "Discipline") +
	theme_minimal()
```

## Charges des variables (lecture des axes)

```{r, label="fig-pca-loadings", fig.cap="ACP : charges des variables sur PC1–PC2 (orientation et importance)."}
arrow_scale <- 3
ggplot(pca_loadings, aes(x = PC1, y = PC2, label = variable)) +
	...
	geom_segment(aes(xend = PC1 * arrow_scale, yend = PC2 * arrow_scale),
					 arrow = grid::arrow(length = unit(0.15, "inches")),
					 alpha = 0.7
	) +
	geom_text(aes(x = PC1 * arrow_scale, y = PC2 * arrow_scale), size = 3, vjust = -0.4) +
	labs(x = "PC1", y = "PC2") +
	coord_equal() +
	theme_minimal()
```

## Visualisation conjointe (optionnel) : facettes par école et stratification par sexe

```{r, label="fig-pca-facets", fig.cap="ACP : PC1–PC2 par discipline, facettée par école, avec stratification par sexe (exploratoire)."}
ggplot(pca_scores, aes(x = PC1, y = PC2, color = sex)) +
	geom_point(alpha = 0.35) +
	facet_grid(subject ~ school) +
	labs(x = "PC1", y = "PC2", color = "Sexe") +
	theme_minimal() +
